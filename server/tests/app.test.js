...
describe('the bloggeropolis endpoints', () => {
    ...
    test('POST /create should create a new user profile', (done) => {
        request(app)
            ...
            .then((res) => {
                ...
                expect(res.body.text).toBe('SUCCESSFUL CREATION');
                done();
            })
            .catch(err => done(err));
    });
    ...
    test('POST /login should allow a user to login', (done) => {
        ...
        .then((res) => {
            expect(res.status).toBe(201);
            expect(res.body.text).toBe('SUCCESSFUL LOGIN');
            done();
        })
        .catch(err => done(err));
    });
    ...
    test('POST /blogs/:userId should allow a post to be created', (done) => {
        request(app)
            ...
            .send({
                ...
                context: 'Whenever I was in junior high school (approximately 10 years ago) I got the most amazing chocolate chip cookie recipe. An older lady from my church found out that I loved to bake and gifted the recipe to me. She told me that I would not be disappointed. And I wasn\'t, they are still to this day my favorite cookie recipe. I have the same notebook paper recipe card she gave me, it\'s now almost unreadable!'
            })
            .then((res) => {
                expect(res.status).toBe(201)
                expect(res.body.text).toBe('BLOG HAS BEEN POSTED')
                done();
            })
            .catch(err => done(err));
    });
    ...
    test('GET /blogs/:userId should retrieve all of the blogs of a user with just 100 char in context', async () => {
        ...
        expect(result.body[0].content).toHaveLength(100);
    });
    ...
    test('GET /blogs/:postId should grab an individual post with complete content', async () => {
        let result = await request(app)
        .get('/blogs/1')
        .expect(200);
    ...
    test('PATCH /blogs/:postId should let the user edit blog content', (done) => {
        ...
        .then((res) => {
            expect(res.status).toBe(201)
            expect(res.body.text).toBe('BLOG HAS BEEN UPDATED')
            done();
        })
        .catch(err => done(err));
    });

    test('GET /blogs/:postId should have the updated post content', async () => {
        let result = await request(app)
        .get('/blogs/1')
        .expect(200);
